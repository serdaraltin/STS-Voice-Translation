...
def set_png_as_page_bg(png_file):
    bin_str = processing.img_to_bytes(png_file)
    page_bg_img = (
        """
   <style>
   body {
...
# Sidebar ---------------------------------------------------------------
with st.sidebar:
    imgSideBar = st.image(resource.path.logo, width=200)
    title = st.markdown(
...
with tab_debug:
    ...
    with col_input_text:
        txt_input_text = st.text_area(value="",key="txt_input_text", label="Source Text", height=300, disabled= True)
...
with tab_panel:
    ...
    with options_save:
        btn_save = st.button(
            key="save", label="SAVE SETTINGS", on_click="", type="primary"
        )
    with options_reset:
        btn_reset = st.button(
            key="reset", label="RESET SETTINGS", on_click=""
        )
    with options_reload:
        btn_reset = st.button(
            key="reload", label="RELOAD SETTINGS", on_click=""
        )
...
with tab_info:
    ...
    st.image(resource.path.logo, width=200)
    st.header("", divider="rainbow")
    info = st.markdown(
